...
def build_pairing(
    ...
    if ios_pairing_id is not None:
        # Regenerated by pair-setup, so a different value models a re-pairing
        # of the same physical accessory.
        pairing_data["iOSPairingId"] = ios_pairing_id
    pairing = CoAPPairing(controller, pairing_data, description=desc)
