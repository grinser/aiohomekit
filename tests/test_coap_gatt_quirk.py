...
def test_the_verdict_does_not_survive_a_re_pairing():
    """An accessory keeps its pairing id across an unpair -- measured: an Eve
    Room kept F2:41:26:0F:C7:2F through a remove and re-pair. So a verdict keyed
    on the accessory alone would outlive the pairing it was formed under, and
    outlive a firmware update installed in between. Our own pairing id is
    regenerated by pair-setup, so a genuine re-pair probes 0x09 again.
    """
    eve = FakeEve()
    before = build_pairing(eve, cached_accessories=cached_map(), ios_pairing_id="controller-a")
    before.connection._gatt_unsupported = True

    after = build_pairing(eve, cached_accessories=cached_map(), ios_pairing_id="controller-b")

    assert not after.connection._gatt_unsupported, (
        "a re-paired accessory inherited a verdict formed under the old pairing"
    )


def test_the_verdict_still_survives_a_fresh_pairing_object():
    """The case the latch exists for must not regress: a removal builds a new
    pairing from a copy of the same credentials, so the id is unchanged.
    """
    eve = FakeEve()
    first = build_pairing(eve, cached_accessories=cached_map(), ios_pairing_id="controller-a")
    first.connection._gatt_unsupported = True

    second = build_pairing(eve, cached_accessories=cached_map(), ios_pairing_id="controller-a")
